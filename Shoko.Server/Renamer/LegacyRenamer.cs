    ...
    public class LegacyRenamer : IRenamer
    {
            ...
            SVR_AniDB_Anime anime)
        {
            try
            {
                ...
                {
                    var title = Repo.Instance.AniDB_Episode_Title.GetByEpisodeIDAndLanguage(episodes[0].EpisodeID, "EN")
                ...
                {
                    var title = Repo.Instance.AniDB_Episode_Title.GetByEpisodeIDAndLanguage(episodes[0].EpisodeID, "X-JAT")
                        .FirstOrDefault()?.Title;
                    if (string.IsNullOrEmpty(title))
                    {
                        return notCondition;
        ...
        public static string GetNewFileName(SVR_VideoLocal_Place place, string script)
        {
            ...
            if (aniFile == null)
            {
                ...
                anime = Repo.Instance.AniDB_Anime.GetByID(episodes[0].AnimeID);
            ...
            SVR_AniDB_File aniFile, List<AniDB_Episode> episodes, SVR_AniDB_Anime anime)
        {
            ...
            {
                var epname = Repo.Instance.AniDB_Episode_Title.GetByEpisodeIDAndLanguage(episodes[0].EpisodeID, "EN")
                    .FirstOrDefault()?.Title;
            ...
            {
                var epname = Repo.Instance.AniDB_Episode_Title.GetByEpisodeIDAndLanguage(episodes[0].EpisodeID, "X-JAT")
                    .FirstOrDefault()?.Title;
                if (epname?.Length > MaxEpisodeNameLength) epname = epname.Substring(0, MaxEpisodeNameLength - 1) + "…";
        ...
        public (ImportFolder dest, string folder) GetDestinationFolder(SVR_VideoLocal_Place video)
        {
            ...
            foreach (SVR_ImportFolder fldr in Repo.Instance.ImportFolder.GetAll()
            ...
            SVR_AnimeSeries series = Repo.Instance.AnimeSeries.GetByAnimeID(xref.AnimeID);
            ...
            foreach (SVR_AnimeEpisode ep in allEps)
            {
                ...
                List<CrossRef_File_Episode> fileEpXrefs =
                    Repo.Instance.CrossRef_File_Episode.GetByEpisodeID(ep.AniDB_EpisodeID);
                int? animeID = null;
